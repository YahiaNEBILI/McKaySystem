...
def test_layout_policy_ignores_generated_egg_info_dir(tmp_path: Path) -> None:
    # Minimal policy-compliant root subset for this isolated test.
    (tmp_path / "apps").mkdir()
    (tmp_path / "docs").mkdir()
    (tmp_path / "tests").mkdir()
    ...
    # Generated by editable installs; should not fail policy checks.
    (tmp_path / "mckaysystem.egg-info").mkdir()
